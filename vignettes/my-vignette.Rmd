...
# 
# timeline_rtweet <- get_timeline("TWITTER_USERNAME", n = 20)
# 
...
#timeline_rtweet <- rtweet::read_twitter_csv("../data/rtweet_raw_df.csv", unflatten = FALSE)
#head(timeline_rtweet)
...
timeline_rtweet <- data.frame(text, retweet_count, favorite_count)
head(timeline_rtweet)
```
...
```{r clean_df_demo}
cleaned_timeline <- rtweetclean::clean_df(timeline_rtweet)
...
head( subset(cleaned_timeline, select=c(text,text_only,word_count,emojis, prptn_rts_vs_avg, prptn_favorites_vs_avg)),3)
```
